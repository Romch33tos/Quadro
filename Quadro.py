...
def Discriminant():
	...
	if (a > 0 and c > 0) or (a < 0 and c < 0):
	  ...
	  text.insert("1.0", f"D = {b}² + 4 × {abs(a)} x {abs(c)}  = {b**2} + {abs(4 * a * c)} = {d}")
	text.insert(END, "\nПо формуле корней:")	
	text.insert(END, f"\nх1 = ({-b} + {int(math.sqrt(d))}) / (2 × {a}) = {int(-b + math.sqrt(d))} / {2 * a} = {int(x1)}")
	text.insert(END, f"\nх2 = ({-b} - {int(math.sqrt(d))}) / (2 × {a}) = {int(-b - math.sqrt(d))} / {2 * a} = {int(x2)}")	
	text.configure(state = DISABLED)
...
def half():
	...
	text.insert(END, f"\nх1 = ({-k} + {int(math.sqrt(d))}) / {a} = {int(-k + math.sqrt(d))} / { a} = {int(x1)}")
	text.insert(END, f"\nх2 = ({-k} - {int(math.sqrt(d))}) / {a} = {int(-k - math.sqrt(d))} / {a} = {int(x2)}")	
	text.configure(state = DISABLED)
...
def Perebroska():
	...
	text.insert(END, f"\nx1 + x2 = {-b}")
	text.insert(END, f"\nx1 × x2 = {c * a}")
	text.insert(END, f"\nx1 = {int(x1 * a)} / {a} = {int(x1)}, x2 = {int(x2 * a)} / {a} = {int(x2)}")
	text.configure(state = DISABLED)
...
D4 = CTkButton(root, width = 255, text = "D/4", command = lambda: half(), font = ("Arial", 30), corner_radius = 10, height = 50, state = DISABLED)
...
N = CTkButton(root, width = 255, text = "Неполное", command = lambda: Discriminant(), font = ("Arial", 30), corner_radius = 10, height = 50, state = DISABLED)
